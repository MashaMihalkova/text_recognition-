    ...
    try:
        for uzel in vertices:
            ...
            text3 = pytesseract.image_to_string(
                image[t1 + round(delta / 2):t2 - round(delta / 2), l1 + round(delta / 2):l2 - round(delta / 2)],
            ...
            if text1 == text2 and text2 == text3:
                ...
                text = text2
            elif text3 != '' and re.sub(' *[^ \-\d\.,]+ *', '', text3) == text3 and (
                    len(text1) + len(text2)) / 2 * 0.4 < len(text3):
                text = text3
...
def recognzie(pathImg, tesseract_cmd, in_json=False):
    ...
    code = decode(img)
    for barcode in code:
        img[barcode.rect.top:barcode.rect.top + barcode.rect.height,
        barcode.rect.left:barcode.rect.left + barcode.rect.width] = np.array([255, 255, 255])
